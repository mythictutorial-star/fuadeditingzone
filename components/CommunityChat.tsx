...
const PasscodeOverlay: React.FC<{ mode: 'set' | 'enter' | 'change'; onSuccess: (pass?: string) => void; onCancel: () => void; storedPasscode?: string; targetName?: string; }> = ({ mode, onSuccess, onCancel, storedPasscode, targetName }) => {
    ...
    const handleDigitInput = (val: string) => {
        ...
        if (clean.length === 4) {
            ...
            setTimeout(() => {
                if (mode === 'set') {
                    if (!tempPass) { setTempPass(code); setDigits([]); } else {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[6000000] bg-black/80 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center" onClick={() => inputRef.current?.focus()}>
            <div className="flex flex-col items-center mb-12 max-w-xs">
                ...
                <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest leading-relaxed">{mode === 'set' ? 'Secure your conversations with a unique 4-digit code' : `Verify identity to access @${targetName?.toLowerCase()}`}</p>
            </div>
            ...
            <div className="flex gap-6 mb-12">{[...Array(4)].map((_, i) => (<div key={i} className={`w-4 h-4 rounded-full border-2 transition-all duration-300 ${digits.length > i ? 'bg-red-600 border-red-600 scale-125 shadow-[0_0_15px_rgba(220,38,38,0.5)]' : 'bg-transparent border-zinc-800'}`}></div>))}</div>
...
export const CommunityChat: React.FC<{ onShowProfile?: (id: string, username?: string) => void; initialTargetUserId?: string | null; onBack?: () => void; onNavigateMarket?: () => void; forceSearchTab?: boolean; onSearchTabConsumed?: () => void; onThreadStateChange?: (active: boolean) => void; }> = ({ onShowProfile, initialTargetUserId, onBack, onNavigateMarket, forceSearchTab, onSearchTabConsumed, onThreadStateChange }) => {
  ...
  useEffect(() => {
    ...
    if (clerkUser) {
        onValue(ref(db, `users/${clerkUser.id}`), (snap) => {
            const data = snap.val();
            if (data) {
                setSettingsData({ name: data.name || '', username: data.username || '', bio: data.profile?.bio || '', gender: data.profile?.gender || 'Prefer not to say' });
  ...
  useEffect(() => {
    ...
    if (blockIds.length > 0) { setBlockedUsersData(users.filter(u => blockIds.includes(u.id))); } else { setBlockedUsersData([]); }
  }, [blockedByMe, users]);
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
        ...
        const newMessage: Message = { senderId: clerkUser.id, senderName: currentUser?.name || clerkUser.fullName || clerkUser.username || "User", senderUsername: (currentUser?.username || clerkUser.username || '').toLowerCase(), senderAvatar: clerkUser.imageUrl, senderRole: currentUser?.role || 'Client', text: text, timestamp: now };
        if (replyingTo) { newMessage.replyTo = { id: replyingTo.id!, text: replyingTo.text, senderName: replyingTo.senderName }; setReplyingTo(null); }
        setInputValue('');
        ...
        if (!isGlobal && selectedUser) {
            ...
            await push(ref(db, `notifications/${selectedUser.id}`), { type: 'message', fromId: clerkUser.id, fromName: currentUser?.name || clerkUser.fullName || clerkUser.username, fromAvatar: clerkUser.imageUrl, text: text, timestamp: now, read: false, isLocked: !!lockedChats[selectedUser.id] });
            ...
            if (!recipientMutedSnap.exists()) { const recipientUnreadRef = ref(db, `users/${selectedUser.id}/unread/${clerkUser.id}`); get(recipientUnreadRef).then(snap => set(recipientUnreadRef, (snap.val() || 0) + 1)); }
        }
  ...
  /**
   * Fix: Implemented togglePushNotifications to handle the toggle switch in UI.
   * Dispatches the 'request-fcm-token' event when enabling, or removes the token from DB when disabling.
   */
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden bg-black font-sans px-4 md:px-0">
                ...
                <aside className={`${isMobileChatOpen ? 'hidden' : 'flex'} md:flex w-full md:w-[320px] flex-col flex-shrink-0 bg-black md:border-r border-white/10`}>
                    <div className="p-6 border-b border-white/5 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-3">
                            <button onClick={() => openOverlay('search')} className="p-2 text-white hover:text-red-500 transition-colors"><SearchIcon className="w-5 h-5" /></button>
                            <button onClick={() => openOverlay('settings')} className="md:hidden p-2 text-white hover:text-red-500 transition-colors"><Settings className="w-5 h-5" /></button>
                        </div>
                    </div>
                        ...
                        {filteredUsers.map(u => (
                            <div key={u.id} onClick={() => openChat(u)} className={`w-full flex items-center gap-4 px-6 py-4 transition-all cursor-pointer relative ${selectedUser?.id === u.id ? 'bg-white/10' : 'hover:bg-white/5'}`}>
                                ...
                                <div className="text-left min-w-0 flex-1">
                                    <p className="text-sm font-bold text-white truncate flex items-center gap-1">{u.name}{getIdentity(u.username)}</p>
                                    <p className="text-[10px] text-zinc-500 truncate">@{u.username.toLowerCase()}</p>
                                </div>
                                {unreadCounts[u.id] > 0 && <div className="bg-red-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full shadow-lg">{unreadCounts[u.id]}</div>}
                            </div>
                        ))}
                    </div>
                </aside>
                <main className={`${isMobileChatOpen ? 'flex' : 'hidden'} md:flex flex-1 flex-col bg-black relative min-w-0 min-h-0`}>
                    {!isGlobal && !selectedUser ? (
                        ...
                        <>
                                ...
                                <div className="flex items-center gap-2 md:gap-3 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 md:gap-3 overflow-hidden">
                                        ...
                                        <div className="min-w-0 cursor-pointer" onClick={() => !isGlobal && onShowProfile?.(selectedUser!.id, selectedUser!.username)}>
                                            <h3 className="text-xs md:text-sm font-black text-white uppercase leading-tight truncate flex items-center gap-1">{isGlobal ? 'Global Stream' : selectedUser?.name}{!isGlobal && getIdentity(selectedUser!.username)}</h3>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 space-y-8 flex flex-col pb-32 no-scrollbar min-w-0 overflow-x-hidden">
                                {messages.map((msg, i) => {
                                    ...
                                    return (
                                        <div key={msg.id || i} className="flex flex-col gap-2 w-full min-w-0">
                                            {showTimeHeader && <div className="text-center py-6"><span className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em]">{formatChatTime(msg.timestamp)}</span></div>}
                                            <div className={`flex gap-2 md:gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'} items-start group w-full min-w-0`}>
                                                ...
                                                <div className={`max-w-[85%] md:max-w-[70%] ${isMe ? 'items-end' : 'items-start'} flex flex-col min-w-0 relative overflow-hidden`}>
                                                    <div className="flex items-center gap-1.5 mb-1 px-1 cursor-pointer w-full" onClick={() => onShowProfile?.(msg.senderId, msg.senderUsername)}>
                                                        <span className="text-[8px] md:text-[10px] font-black text-white uppercase tracking-tight truncate max-w-[80px] md:max-w-[100px]">{msg.senderName}</span>
                                                        {getIdentity(msg.senderUsername || '')}
                                                        <span className="text-[6px] md:text-[7px] font-black text-red-600/60 uppercase tracking-widest bg-red-600/5 px-1 py-0.5 rounded border border-red-600/10 flex-shrink-0">{msg.senderRole || 'Member'}</span>
                                                    </div>
                                                    <div className="relative group/bubble flex flex-col items-inherit w-full min-w-0 overflow-hidden">
                                                      {msg.replyTo && <div className={`mb-[-12px] pb-4 pt-2 px-3 rounded-t-xl bg-white/5 border-t border-x border-white/10 text-[9px] text-zinc-500 max-w-full truncate ${isMe ? 'self-end' : 'self-start'}`}><p className="font-black uppercase tracking-widest text-[7px] text-red-600/60 mb-0.5">Reply to {msg.replyTo.senderName}</p><p className="italic opacity-60 truncate">{msg.replyTo.text}</p></div>}
                                                      <div className={`px-3.5 py-2.5 md:px-4 md:py-2.5 rounded-xl md:rounded-2xl text-[11px] md:text-sm leading-relaxed break-words whitespace-pre-wrap overflow-hidden ${isMe ? 'bg-red-600 text-white rounded-tr-none self-end text-right' : 'bg-[#1a1a1a] text-zinc-200 rounded-tl-none border border-white/5 self-start text-left'}`}>{msg.text}</div>
                                                      <button onClick={() => setReplyingTo(msg)} className={`absolute top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/5 opacity-0 group-hover/bubble:opacity-100 transition-all hover:bg-white/10 ${isMe ? 'right-full mr-2' : 'left-full ml-2'}`}><Reply size={14} className="text-zinc-500" /></button>
                                                    </div>
                                                    <span className={`text-[7px] md:text-[8px] font-bold text-zinc-700 uppercase mt-1 tracking-widest ${isMe ? 'text-right' : 'text-left'}`}>{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                                </div>
                                            </div>
                                        </div>
                                    );
                            ...
                            <div className="px-2 pt-2 pb-3 md:px-10 md:pb-8 bg-black border-t border-white/5 flex-shrink-0 z-50">
                                <AnimatePresence>{replyingTo && (<motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="max-w-4xl mx-auto mb-2 p-3 bg-[#111] border border-white/5 rounded-xl flex items-center justify-between group"><div className="flex items-center gap-3 overflow-hidden"><div className="w-1 h-8 bg-red-600 rounded-full flex-shrink-0"></div><div className="min-w-0"><p className="text-[9px] font-black text-red-600 uppercase tracking-widest">Replying to {replyingTo.senderName}</p><p className="text-[11px] text-zinc-500 truncate">{replyingTo.text}</p></div></div><button onClick={() => setReplyingTo(null)} className="p-2 text-zinc-600 hover:text-white transition-colors"><X size={16} /></button></motion.div>)}</AnimatePresence>
                                <form onSubmit={handleSendMessage} className="w-full max-w-4xl mx-auto flex items-end gap-2 p-1 bg-[#0a0a0a] border border-white/10 rounded-2xl min-h-[46px] md:min-h-[50px] focus-within:border-red-600/40 transition-all overflow-hidden"><textarea ref={textareaRef} value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyDown={e => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(); }}} placeholder="Type message..." rows={1} className="flex-1 bg-transparent px-4 py-2.5 text-[13px] md:text-sm text-white outline-none resize-none placeholder-zinc-700 max-h-32 min-w-0" /><button type="submit" disabled={!inputValue.trim()} className="px-4 md:px-5 py-2 md:py-2.5 text-red-600 font-black uppercase text-[9px] md:text-[10px] tracking-widest disabled:opacity-20 transition-all flex-shrink-0">Send</button></form>
                            </div>
                        </>
                    )}
                </main>
              ...
              <motion.div initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} className="fixed inset-0 z-[10000000] bg-black backdrop-blur-2xl flex flex-col p-4 md:p-20 overflow-y-auto custom-scrollbar pb-32 md:pb-20">
                  <div className="max-w-3xl mx-auto w-full flex flex-col h-full">
                      <div className="flex items-center justify-between mb-6 md:mb-12">
                          <div className="flex items-center gap-3"><Settings className="w-6 h-6 md:w-8 md:h-8 text-red-600" /><h2 className="text-xl md:text-5xl font-black text-white uppercase tracking-tighter">Settings</h2></div>
                      ...
                      <div className="px-1 text-[11px] md:text-sm">
                      {settingsTab === 'profile' ? (
                          <div className="space-y-5 md:space-y-8 animate-fade-in">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                  <div className="space-y-1.5 md:space-y-3"><label className="text-[8px] font-black text-zinc-600 uppercase tracking-widest ml-1">Display Name</label><input value={settingsData.name} onChange={e => setSettingsData({...settingsData, name: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white text-xs font-bold outline-none focus:border-red-600/50 transition-all" /></div>
                                  <div className="space-y-1.5 md:space-y-3"><label className="text-[8px] font-black text-zinc-600 uppercase tracking-widest ml-1">Username</label><input value={settingsData.username} onChange={e => setSettingsData({...settingsData, username: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white text-xs font-bold outline-none focus:border-red-600/50 transition-all lowercase" /></div>
                              </div>
                              <div className="space-y-1.5 md:space-y-3">
                                  <label className="text-[8px] font-black text-zinc-600 uppercase tracking-widest ml-1">Gender</label>
                                  <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">{['Male', 'Female', 'Secret'].map((g) => (<button key={g} onClick={() => setSettingsData({...settingsData, gender: g})} className={`flex-1 py-2 px-3 rounded-xl border text-[8px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${settingsData.gender === g ? 'bg-white text-black border-white' : 'bg-white/5 border-white/5 text-zinc-500 hover:text-white'}`}>{g}</button>))}</div>
                              </div>
                              <div className="space-y-1.5 md:space-y-3"><label className="text-[8px] font-black text-zinc-600 uppercase tracking-widest ml-1">Short Bio</label><textarea value={settingsData.bio} onChange={e => setSettingsData({...settingsData, bio: e.target.value})} rows={2} className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white text-[11px] md:text-sm font-medium outline-none focus:border-red-600/50 transition-all resize-none" placeholder="Bio..." /></div>
                              <button onClick={() => { update(ref(db, `users/${clerkUser?.id}`), settingsData); openOverlay('inbox'); }} className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-4 md:py-6 rounded-2xl md:rounded-3xl uppercase tracking-[0.3em] text-[9px] md:text-xs shadow-xl transition-all active:scale-95">Save Profile</button>
                          </div>
                      ) : (<div className="space-y-6 animate-fade-in pb-10"><div className="space-y-3"><h4 className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.3em] px-1">Notifications</h4><div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10"><div className="flex items-center gap-3"><BellRing className={pushEnabled ? "text-red-500" : "text-zinc-500"} size={16} /><div className="text-left"><p className="text-xs font-bold text-white leading-tight">Push Alerts</p><p className="text-[8px] text-zinc-500 uppercase tracking-widest mt-0.5">Stay notified</p></div></div><button onClick={togglePushNotifications} className={`w-10 h-5 md:w-12 md:h-6 rounded-full transition-all relative flex-shrink-0 ${pushEnabled ? 'bg-red-600' : 'bg-zinc-800'}`}><motion.div animate={{ x: pushEnabled ? 22 : 2 }} className="absolute top-0.5 md:top-1 left-0 w-4 h-4 bg-white rounded-full" /></button></div></div></div>)}
                      </div>
                  </div>
              </motion.div>
          )}
      ...
      <AnimatePresence>
          {isSearchOverlayOpen && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="fixed inset-0 z-[10000000] bg-black/80 backdrop-blur-xl flex flex-col" >
                  <div className="max-w-4xl mx-auto w-full flex flex-col h-full p-6 md:p-20 overflow-hidden">
                      <div className="flex items-center justify-between mb-8 md:mb-16 flex-shrink-0"><h2 className="text-2xl md:text-6xl font-black text-white uppercase tracking-tighter">Explore</h2><button onClick={() => openOverlay('inbox')} className="p-2.5 md:p-4 bg-white/5 rounded-full hover:bg-red-600 transition-all"><CloseIcon className="w-5 h-5 md:w-8 md:h-8 text-white" /></button></div>
                      <div className="relative mb-10 md:mb-16 flex-shrink-0"><SearchIcon className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-600 w-6 h-6" /><input autoFocus value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Type name or handle..." className="w-full bg-white/5 border border-white/10 rounded-2xl md:rounded-[2.5rem] py-6 md:py-8 pl-16 md:pl-20 pr-8 text-xl md:text-3xl font-bold text-white outline-none focus:border-red-600 transition-all placeholder-zinc-800" /></div>
                      <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 md:space-y-6">
                          {searchResults.length === 0 ? (<div className="py-20 text-center opacity-20"><SearchIcon className="w-16 h-16 mx-auto mb-4" /><p className="text-[10px] font-black uppercase tracking-[0.5em]">Nothing found</p></div>) : searchResults.map(u => (<div key={u.id} onClick={() => openChat(u)} className="p-4 md:p-8 bg-white/[0.02] border border-white/5 rounded-2xl md:rounded-[2.5rem] flex items-center justify-between hover:bg-red-600/10 hover:border-red-600/20 transition-all cursor-pointer group"><div className="flex items-center gap-4 md:gap-8"><UserAvatar user={u} className="w-12 h-12 md:w-24 md:h-24" onClick={(e) => { e.stopPropagation(); onShowProfile?.(u.id, u.username); }} /><div className="text-left min-w-0"><div className="flex items-center gap-2"><p className="text-base md:text-3xl font-black text-white uppercase tracking-tight truncate">{u.name}</p>{getIdentity(u.username)}</div><p className="text-[9px] md:text-sm text-zinc-500 font-bold uppercase tracking-widest mt-0.5 md:mt-2">@{u.username.toLowerCase()} • {u.role || 'Member'}</p></div></div><ChevronRightIcon className="w-6 h-6 md:w-10 md:h-10 text-zinc-800 group-hover:text-red-600 transition-all" /></div>))}
                      </div>
                  </div>
              </motion.div>
          )}
      ...
      <AnimatePresence>
          {isActivityOpen && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[10000000] bg-black/95 backdrop-blur-xl flex flex-col p-6 md:p-20 overflow-y-auto"><div className="max-w-3xl mx-auto w-full flex flex-col h-full"><div className="flex items-center justify-between mb-12"><div className="flex items-center gap-4"><Bell className="w-8 h-8 text-red-600" /><h2 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tighter">Activity</h2></div><button onClick={() => openOverlay('inbox')} className="p-3 bg-white/5 rounded-full hover:bg-red-600 transition-all"><CloseIcon className="w-6 h-6 text-white" /></button></div><div className="flex-1 space-y-4">{notifications.length === 0 ? (<div className="py-32 text-center opacity-20"><Bell size={100} className="mx-auto mb-6" /><p className="text-sm md:text-xl font-black uppercase tracking-[0.5em]">No alerts yet</p></div>) : (notifications.map((n) => (<div key={n.id} className="p-6 bg-white/[0.03] border border-white/5 rounded-2xl flex items-center gap-6 group hover:bg-white/10 transition-all cursor-pointer"><img src={n.fromAvatar} className="w-14 h-14 rounded-2xl object-cover border border-white/10" onClick={(e) => { e.stopPropagation(); onShowProfile?.(n.fromId); }} /><div className="flex-1" onClick={() => onShowProfile?.(n.fromId)}><p className="text-sm md:text-lg text-gray-200"><span className="font-black text-white">@{ (n.fromName || '').toLowerCase() }</span> {n.text || 'pinged you.'}</p><p className="text-[10px] text-zinc-600 font-bold uppercase mt-2 tracking-widest">{new Date(n.timestamp).toLocaleString()}</p></div></div>)))}</div></div></motion.div>
          )}
      </AnimatePresence>
